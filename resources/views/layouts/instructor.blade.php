...
<head>
  ...
  <title>@yield('title','Instructor') — Crygle Studio</title>
  ...
  <aside class="w-64 bg-slate-900 text-white flex-shrink-0 flex flex-col">
    <div class="px-5 py-5 border-b border-slate-800">
      <div class="flex items-center gap-2.5">
        <div class="w-8 h-8 bg-purple-600 rounded-lg flex items-center justify-center font-bold text-sm">C</div>
        <div><p class="font-bold text-sm">Crygle</p><p class="text-xs text-slate-400">Instructor Studio</p></div>
      </div>
    </div>
    <nav class="flex-1 px-3 py-5 space-y-0.5">
      <a href="{{ route('instructor.dashboard') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition {{ request()->routeIs('instructor.dashboard') ? 'bg-purple-700 text-white font-semibold' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
        <i class="bi-grid-1x2"></i> Dashboard
      </a>
      <a href="{{ route('instructor.courses.index') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition {{ request()->routeIs('instructor.courses.*') ? 'bg-purple-700 text-white font-semibold' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}">
        <i class="bi-collection-play"></i> Course Saya
      </a>
    ...
    <div class="px-4 py-4 border-t border-slate-800">
      <div class="flex items-center gap-2.5">
        ...
        <div class="flex-1 min-w-0"><p class="text-sm font-medium truncate">{{ auth()->user()->name }}</p><p class="text-xs text-slate-400">Instructor</p></div>
